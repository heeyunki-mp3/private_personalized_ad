...
int main(int argc, char *argv[])
{
    ...
    while((n = read(sockfd, buffer, 255))==255){
        paramString.append(buffer);
        std::cout << "buffer: "<< paramString<< endl;
        hexDump(buffer, 255);
    }
	...
	std::cout << "Main: Initializing pir client" << endl;
    const char *param_char = paramString.c_str();
	::memcpy(enc_param_buffer, param_char, sizeof(EncryptionParameters));
	::memcpy(pir_param_buffer, param_char +sizeof(enc_params), sizeof(PirParams));

	std::cout << "Main: copied params" << endl;

	print_pir_params(*(PirParams *)pir_param_buffer);
